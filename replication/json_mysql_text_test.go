...
// serverParityCorpus covers every decimal point position in [-18, 18]
// crossed with 1/2/16/17 significant digits and both signs, plus the
// pinned vectors and a pseudo-random spread. The generator is a
// fixed-seed xorshift so the corpus is identical on every run.
...
// TestFormatMySQLDoubleServerParity asks a live server to render every
// corpus value as a JSON double and requires byte identity. Skips when
