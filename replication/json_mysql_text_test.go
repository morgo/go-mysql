...
// TestRenderJSONAsMySQLTextOpaqueZeroTemporals pins the zero values.
...
// A zero shortcut in decodeTime/decodeDateTime used to drop the
// fraction, so text-mediated writes stored a different string than the
// server's own render of the same JSONB value.
